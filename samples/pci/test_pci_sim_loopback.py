...
"""Basic host-side loopback test for samples/pci/fake_pci_sriov.ko.
...
def find_pf():
    for dev in sorted(Path("/sys/bus/pci/devices").iterdir()):
        try:
            if read_text(dev / "vendor") == VENDOR and read_text(dev / "device") == PF_DEVICE:
                return dev
        except FileNotFoundError:
            continue
    return None
...
def remove_fake_pf(timeout):
    ...
    try:
        ...
        print(f"warning: failed to disable VFs before remove: {exc}", file=sys.stderr)
...
def test_tty(path, payload, timeout):
    ...
    try:
        ...
        if written != len(payload):
            raise RuntimeError(f"short write to {path}: {written}/{len(payload)}")

        got = read_exact(fd, len(payload), timeout)
        if got != payload:
            raise RuntimeError(f"loopback mismatch on {path}: got {got!r}, expected {payload!r}")
    finally:
...
def main():
    ...
    parser.add_argument("--module", default="samples/pci/fake_pci_sriov.ko",
                        help="module path for --load/--reload")
    parser.add_argument("--load", "--insmod", dest="load", action="store_true",
    ...
    parser.add_argument("--unload", "--rmmod", dest="unload", action="store_true",
                        help="remove fake PF and rmmod the module after testing")
    parser.add_argument("--vfs", type=int, default=2, help="number of VFs to enable")
    parser.add_argument("--timeout", type=float, default=5.0, help="wait/read timeout in seconds")
    parser.add_argument("--keep-vfs", action="store_true", help="leave VFs enabled after the test")
    ...
    try:
        ...
        if args.unload:
            ...
            try:
                ...
                print(f"warning: failed to disable VFs: {exc}", file=sys.stderr)
