...
#define FAKE_PCI_VENDOR_ID	0x1b36	/* QEMU's vendor ID */
#define FAKE_PCI_PF_DEVICE_ID	0x000c	/* PF device ID */
#define FAKE_PCI_VF_DEVICE_ID	0x000d	/* VF device ID */
#define FAKE_PCI_CLASS		0x070002 /* Serial controller, 16550 */
#define FAKE_PCI_SUBSYS_VENDOR	0x1b36
#define FAKE_PCI_SUBSYS_ID	0x000c
...
/* Static resource for PCI bus (must not be stack-allocated) */
...
static void init_pf_config_space(struct fake_pci_device *dev)
{
	...
	*(u16 *)&config[PCI_STATUS] = PCI_STATUS_CAP_LIST;
	...
	config[PCI_CLASS_PROG] = 0x02;	/* 16550 compatible */
	config[PCI_CLASS_DEVICE] = 0x00;
	config[PCI_CLASS_DEVICE + 1] = 0x07;
...
{
	struct fake_pci_host *host = bus->sysdata;
				 ...
				 int where, int size, u32 val)
{
	...
	/* Handle SR-IOV NumVFs write (PF only) */
	if (!dev->is_vf && where == SRIOV_CAP_OFFSET + PCI_SRIOV_NUM_VF) {
		u16 num = val & 0xFFFF;

		handle_sriov_numvfs_write(host, num);
		return PCIBIOS_SUCCESSFUL;
	}

	/* Handle SR-IOV Control write */
	if (!dev->is_vf && where == SRIOV_CAP_OFFSET + PCI_SRIOV_CTRL) {
		u16 ctrl = val & 0xFFFF;
		u16 num_vfs = *(u16 *)&config[SRIOV_CAP_OFFSET + PCI_SRIOV_NUM_VF];

		/* Store control value */
		*(u16 *)&config[where] = ctrl;

		/* If VF Enable is set and we have VFs requested, enable them */
		if ((ctrl & PCI_SRIOV_CTRL_VFE) && num_vfs > 0) {
			if (host->num_vfs_enabled != num_vfs)
				handle_sriov_numvfs_write(host, num_vfs);
		} else if (!(ctrl & PCI_SRIOV_CTRL_VFE)) {
			/* VF Enable cleared - disable all VFs */
			if (host->num_vfs_enabled > 0)
...
static void handle_sriov_numvfs_write(struct fake_pci_host *host, u16 num_vfs)
{
	...
	*(u16 *)&pf_config[SRIOV_CAP_OFFSET + PCI_SRIOV_NUM_VF] = num_vfs;
	...
	/* Rescan the bus to pick up new VFs or remove old ones */
	if (host->bridge && host->bridge->bus) {
		pci_lock_rescan_remove();
		pci_rescan_bus(host->bridge->bus);
